...
const RecentCard: React.FC<CardProps> = ({ item }) => {
  return (
    <div className="card hover:border-2 hover:border-slate-900 flex flex-col  gap-[20px] p-4 rounded-lg shadow-lg bg-white hover:shadow-2xl transition-shadow duration-200">
      {/* Image Container */}
      <Link
        href={`/${item.id}`}
        className="imageContainer h-[200px] w-full relative"
      >
        <img
          src={item.img}
          alt={item.title}
            ...
            <div className="icons flex  gap-4">
              <div className="icon cursor-pointer p-2 border border-[#999] rounded-md flex items-center justify-center hover:bg-gray-200 transition-colors duration-200 ease-in-out">
                <img
                  src="/save.png"
                  alt="save"
                  className="w-[16px] h-[16px] sm:w-2 sm:h-2"
                />
              </div>
            </div>
          </div>
